...
import "./CheckBalanceDetailsMerchant"
...
import CurrentBalanceDetailsAllData from '../../AdminePanel/AdminSearchUserId/CurrentBalanceDetails/CurrentBalanceDetailsAllData/CurrentBalanceDetailsAllData';

const CheckBalanceDetailsMerchant = () => {

    let navigate = useNavigate()
    const [roles] = useRole()
    let { email, _id, Address, BusinessName, LastName, Password, Phone, name, photo, role, userId, Districts, PoliceStations, date, status, RoutePasswordDetails } = roles
    ...
    // ===================================================
    // User All Parcel Data get bellow !!
    // ===================================================
    let { refetch, data: AllParcelData = [] } = useQuery(["userRoleCheck"], async () => {
        let res = await fetch(`https://server.trustereocourier.com.bd/AdminCurrentBalanceAllData?email=${email}`)
        return res.json()
    })
    ...
    let CodAmountPaymentData = AllParcelData?.filter(PaymentAll => PaymentAll?.Payment == "Yes" && PaymentAll?.status == "Delivered" || PaymentAll?.Payment == "Yes" && PaymentAll?.status == "PartiallyDelivered")
...
export default CheckBalanceDetailsMerchant;